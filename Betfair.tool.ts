...
export class BetfairTool extends Tool {
	...
	async _call(rawInput: string | { input: string } | Record<string, unknown>): Promise<string> {
		// Newer @langchain/core versions may pass an object instead of a plain string
		const input: string = typeof rawInput === 'string' ? rawInput : ((rawInput as any).input ?? JSON.stringify(rawInput));
