...
class BetfairAus {
    ...
    async execute() {
        ...
        if (!credentials) {
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'No credentials found. Please add Betfair Australia API credentials.');
        }
        ...
        try {
            ...
            if (loginResponse.data?.token) {
                ...
                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Betfair login failed: ${loginResponse.data?.error || loginResponse.data?.loginStatus || 'No token received'}`);
            }
        }
        catch (error) {
            if (error instanceof n8n_workflow_1.NodeOperationError)
                throw error;
            if (axios_1.default.isAxiosError(error) && error.response) {
                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Betfair login API error: ${JSON.stringify(error.response.data)}`);
            }
            throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Betfair login request error: ${error.message}`);
        }
        ...
        for (let itemIndex = 0; itemIndex < executionItems.length; itemIndex++) {
            try {
                ...
                const getArrayParam = (name, collection) => {
                    const raw = collection[name] || '';
                    return raw.trim() ? raw.split(',').map((item) => item.trim()).filter(Boolean) : undefined;
                };
                ...
                if (['listEventTypes', 'listEvents', 'listVenues', 'listMarketTypes', 'listCompetition', 'listMarketCatalogue'].includes(operation)) {
                    const filterOptions = this.getNodeParameter('filters', itemIndex, {});
                    const filter = {};
                    ...
                    if (filterOptions.filterTextQuery)
                        filter.textQuery = filterOptions.filterTextQuery;
                    if (filterOptions.filterBspOnly)
                        filter.bspOnly = true;
                    if (filterOptions.filterTurnInPlayEnabled)
                        filter.turnInPlayEnabled = true;
                    if (filterOptions.filterInPlayOnly)
                        filter.inPlayOnly = true;
                    if (filterOptions.filterMarketBettingTypes)
                        filter.marketBettingTypes = [filterOptions.filterMarketBettingTypes];
                    if (filterOptions.filterWithOrders)
                        filter.withOrders = [filterOptions.filterWithOrders];
                    if (filterOptions.filterRaceTypes && filterOptions.filterRaceTypes !== 'NO_VALUE')
                        filter.raceTypes = [filterOptions.filterRaceTypes];
                    if (filterOptions.filterMarketStartFrom || filterOptions.filterMarketStartTo) {
                        filter.marketStartTime = {};
                        if (filterOptions.filterMarketStartFrom)
                            filter.marketStartTime.from = new Date(filterOptions.filterMarketStartFrom).toISOString();
                        if (filterOptions.filterMarketStartTo)
                            filter.marketStartTime.to = new Date(filterOptions.filterMarketStartTo).toISOString();
                    }
                    requestBody.filter = filter;
                    if (operation === 'listMarketCatalogue') {
                        const catOptions = this.getNodeParameter('marketCatalogueOptions', itemIndex, {});
                    ...
                    const marketIdsRaw = this.getNodeParameter('filterMarketIds', itemIndex, '');
                    const marketIds = marketIdsRaw.trim() ? marketIdsRaw.split(',').map(id => id.trim()).filter(Boolean) : undefined;
                    if (!marketIds || marketIds.length === 0) {
                        throw new n8n_workflow_1.NodeOperationError(this.getNode(), 'Market IDs are required for List Market Book(s).', { itemIndex });
                    }
                    const options = this.getNodeParameter('marketBookOptions', itemIndex, {});
                    requestBody = { marketIds };
                    const priceProjection = {};
                    if (options.priceProjectionPriceData)
                        priceProjection.priceData = options.priceProjectionPriceData;
                    if (options.priceProjectionVirtualise)
                        priceProjection.virtualise = options.priceProjectionVirtualise;
                    if (options.priceProjectionExBestOffersDepth)
                        priceProjection.exBestOffersOverrides = { bestPricesDepth: options.priceProjectionExBestOffersDepth };
                    if (Object.keys(priceProjection).length > 0)
                        requestBody.priceProjection = priceProjection;
                    if (options.orderProjection && options.orderProjection !== 'OMIT')
                        requestBody.orderProjection = options.orderProjection;
                    if (options.matchProjection && options.matchProjection !== 'OMIT')
                        requestBody.matchProjection = options.matchProjection;
                }
                else if (operation === 'listCurrentOrders') {
                    const options = this.getNodeParameter('currentOrdersOptions', itemIndex, {});
                    const getArrayParamOrders = (name) => {
                    ...
                    if (placedDateFrom || placedDateTo) {
                        ...
                        if (placedDateFrom)
                            requestBody.placedDateRange.from = new Date(placedDateFrom).toISOString();
                        if (placedDateTo)
                            requestBody.placedDateRange.to = new Date(placedDateTo).toISOString();
                    }
                }
                else {
                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Unknown or unhandled operation '${operation}'`, { itemIndex });
                }
                // Make the API call
