...
function extractReferencesInNodeExpressions(subGraph, nodeNames, insertedStartName, graphInputNodeName) {
    if (nodeNames.includes(insertedStartName))
        throw new errors_1.OperationalError(`StartNodeName ${insertedStartName} already exists in nodeNames: ${JSON.stringify(nodeNames)}`);
    ...
    const recMapByNode = new Map();
    const allData = [];
    for (const node of subGraph) {
        const [parameterMapping, allMappings] = applyParameterMapping(node.parameters, (s) => parseReferencingExpressions(s, nodeRegexps, nodeNames, insertedStartName, node.name === graphInputNodeName));
    ...
    const output = [];
    for (const node of subGraph) {
        const { result, usedMappings } = applyExtractMappingToNode((0, lodash_1.cloneDeep)(node), recMapByNode.get(node.name));
